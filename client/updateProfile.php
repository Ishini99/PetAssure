...
if (isset($_POST["submit"])) {
    ...
    if ($_FILES["image"]["error"] == 4) {
        ...
        if (!in_array($imageExtension, $validImageExtension)) {
            ...
            // 
            $query1 ="DELETE FROM user_images WHERE uid = '$userid'";
            mysqli_query($con, $query1);
            //      
                $query = "INSERT INTO user_images (name, image,uid) VALUES('$name', '$newImageName','$userid')";
...
if (isset($_POST["delete"])) {
    ...
    if (!isset($imgId) || !is_numeric($imgId)) {
        ...
        $query = "DELETE FROM user_image WHERE imgId = $imgId";
        mysqli_query($con, $query);
        unlink("user_img/" . $imageName); // delete the image file from server
