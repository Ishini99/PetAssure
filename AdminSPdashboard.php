    ...
    <button class="sbutton" type="submit" name="submit">Search</button>
     ...
     $resultcheck2 = mysqli_num_rows($result2);
      ...
      if (!empty($search)) {
        ...
        $sql = "SELECT user.userid, user.fname, user.email, user.mobile, serviceprovider.stype FROM `serviceprovider` INNER JOIN `user` ON serviceprovider.userid = user.userid  WHERE `fname` LIKE '%$search%'";
      } else {
        // If search query is empty, show all records
        $sql = "SELECT user.userid, user.fname, user.email, user.mobile, serviceprovider.stype FROM `serviceprovider` INNER JOIN `user` ON serviceprovider.userid = user.userid;";
      }
    } else {
