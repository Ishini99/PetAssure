...
if(isset($_POST["submit"])){
    ...
    if($_FILES["image"]["error"] == 4){
      ...
      if ( !in_array($imageExtension, $validImageExtension) ){
        ...
        move_uploaded_file($tmpName, 'sitter_img/' . $newImageName);
        $query = "INSERT INTO sitimage (name, image,uid) VALUES('$name', '$newImageName','$userid')";
...
if(isset($_POST["delete"])){
    ...
    if (!isset($imgId) || !is_numeric($imgId)) {
        ...
        $query = "DELETE FROM sitimage WHERE imgId = $imgId";
        mysqli_query($con, $query);
        unlink("sitter_img/" . $imageName); // delete the image file from server
